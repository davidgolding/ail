...
2. `/ail-plan $ARGUMENTS` — **Record the plan file path** from `docs/plans/` for steps 4 and 6.
3. `/ail-work` — **Use swarm mode**: Make a Task list and launch an army of agent swarm subagents to build the plan
...
6. `/ail-review mode:autofix plan:<plan-path-from-step-2>` — run sequentially after the parallel phase so it can safely mutate the checkout, apply `safe_auto` fixes, and emit residual todos for step 7
