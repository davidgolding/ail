...
Scan `.ai/todos/*.md`. Partition by status:
...
Residual actionable work from `ail-review mode:autofix` after its `safe_auto` pass will already be `ready`.
...
Spawn a `ail:workflow:pr-comment-resolver` agent per item. Prefer parallel; fall back to sequential respecting dependency order.
...
For large sets, use a scratch directory at `.context/ail/todo-resolve/<run-id>/` for per-resolver artifacts. Return only completion summaries to parent.
...
Load the `ail-compound` skill to document what was learned. Todo resolutions often surface patterns and architectural insights worth capturing.
...
Delete completed/resolved todo files from `.ai/todos/`. If a scratch directory was created at `.context/ail/todo-resolve/<run-id>/`, delete it (unless user asked to inspect).
