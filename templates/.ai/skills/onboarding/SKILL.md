...
Read files in parallel batches where there are no dependencies between them. For example, batch README.md, entry points, and .ai/system.md/.ai/system.md together in a single turn since none depend on each other's content.
...
5. **.ai/system.md or .ai/system.md** (if exists) -- for project conventions and patterns already documented.
...
**Width constraint for code blocks -- 80 columns max.** Markdown code blocks render with `white-spaail- pre` and never wrap, so wide lines cause horizontal scrolling on GitHub, tablets, and narrow viewports. Tables are fine -- markdown renderers wrap them. Apply these rules to all content inside ``` fences:
