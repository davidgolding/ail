...
2. `/ail-plan $ARGUMENTS`

   GATE: STOP. Verify that the `ail-plan` workflow produced a plan file in `docs/plans/`. If no plan file was created, run `/ail-plan $ARGUMENTS` again. Do NOT proceed to step 3 until a written plan exists. **Record the plan file path** — it will be passed to ail-review in step 4.
...
4. `/ail-review mode:autofix plan:<plan-path-from-step-2>`
