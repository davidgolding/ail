...
- `ail:workflow:spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `ail:research:repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `ail:research:learnings-researcher` for institutional knowledge and past solved problems
- `ail:research:framework-docs-researcher` for official framework or library behavior
- `ail:research:best-practices-researcher` for current external patterns and industry guidance
- Add `ail:research:git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `ail:review:architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `ail:research:framework-docs-researcher` or `ail:research:best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `ail:review:architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `ail:research:repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `ail:research:best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `ail:research:repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `ail:review:pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `ail:workflow:spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `ail:review:architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `ail:review:performance-oracle` for scalability, latency, throughput, and resource-risk analysis
  - `ail:review:security-sentinel` for auth, validation, exploit surfaces, and security boundary review
  - `ail:review:data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `ail:review:security-sentinel` for security, auth, privacy, and exploit risk
  - `ail:review:data-integrity-guardian` for persistent data safety, constraints, and transaction boundaries
  - `ail:review:data-migration-expert` for migration realism, backfills, and production data transformation risk
  - `ail:review:deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `ail:review:performance-oracle` for capacity, latency, and scaling concerns
...
Artifact-backed mode uses a per-run scratch directory under `.context/ail/ail-plan/deepen/`.
...
- Recommend `ail-brainstorm` if the gap is truly product-defining