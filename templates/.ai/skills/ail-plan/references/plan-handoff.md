...
**Question:** "Plan ready at `plans/YYYY-MM-DD-NNN-<type>-<name>-plan.md`. What would you like to do next?"

**Options:**
1. **Start `/ail-work`** - Begin implementing this plan in the current environment (recommended)
...
5. **Start `/ail-work` in another session** - Begin implementing in a separate agent session when the current platform supports it
6. **Create Issue** - Create an issue in the configured tracker

Based on selection:
- **Open plan in editor** -> Open `plans/<plan_filename>.md` using the current platform's file-open or editor mechanism (e.g., `open` on macOS, `xdg-open` on Linux, or the IDE's file-open API)
...
- **Share to Proof** -> Upload the plan:
  ```bash
  CONTENT=$(cat plans/<plan_filename>.md)
...
- **`/ail-work`** -> Call `/ail-work` with the plan path
- **`/ail-work` in another session** -> If the current platform supports launching a separate agent session, start `/ail-work` with the plan path there. Otherwise, explain the limitation briefly and offer to run `/ail-work` in the current session instead.
- **Create Issue** -> Follow the Issue Creation section below
- **Other** -> Accept free text for revisions and loop back to options

## Issue Creation

When the user selects "Create Issue", detect their project tracker from `.ai/system.md` or `README.md`:
...
4. If no tracker is configured:
   ...
   - Suggest adding the tracker to `README.md` for future runs

After issue creation:
- Display the issue URL
- Ask whether to proceed to `/ail-work`