...
| `plan:<path>` | `plan:plans/2026-03-25-001-feat-foo-plan.md` | Load this plan for requirements verification |
...
- **Write a run artifact** under `.context/ail/ail-review/<run-id>/` summarizing findings, applied fixes, residual actionable work, and advisory outputs.
...
- **Never edit files or externalize work.** Do not write `.context/ail/ail-review/<run-id>/`, do not create todo files, and do not commit, push, or create a PR.
...
- **Write a run artifact** under `.context/ail/ail-review/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
| Agent | Focus |
|-------|-------|
| `ail:review:correctness-reviewer` | Logic errors, edge cases, state bugs, error propagation |
| `ail:review:testing-reviewer` | Coverage gaps, weak assertions, brittle tests |
| `ail:review:maintainability-reviewer` | Coupling, complexity, naming, dead code, abstraction debt |
| `ail:review:project-standards-reviewer` | .ai/system.md compliance -- frontmatter, references, naming, portability |
| `ail:review:agent-native-reviewer` | Verify new features are agent-accessible |
| `ail:research:learnings-researcher` | Search docs/solutions/ for past issues related to this PR |

**Cross-cutting conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `ail:review:security-reviewer` | Auth, public endpoints, user input, permissions |
| `ail:review:performance-reviewer` | DB queries, data transforms, caching, async |
| `ail:review:api-contract-reviewer` | Routes, serializers, type signatures, versioning |
| `ail:review:data-migrations-reviewer` | Migrations, schema changes, backfills |
| `ail:review:reliability-reviewer` | Error handling, retries, timeouts, background jobs |
| `ail:review:adversarial-reviewer` | Diff >=50 changed non-test/non-generated/non-lockfile lines, or auth, payments, data mutations, external APIs |
| `ail:review:cli-readiness-reviewer` | CLI command definitions, argument parsing, CLI framework usage, command handler implementations |
| `ail:review:previous-comments-reviewer` | Reviewing a PR that has existing review comments or threads |

**Stack-specific conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `ail:review:dhh-rails-reviewer` | Rails architecture, service objects, session/auth choices, or Hotwire-vs-SPA boundaries |
| `ail:review:kieran-rails-reviewer` | Rails application code where conventions, naming, and maintainability are in play |
| `ail:review:kieran-python-reviewer` | Python modules, endpoints, scripts, or services |
| `ail:review:kieran-typescript-reviewer` | TypeScript components, services, hooks, utilities, or shared types |
| `ail:review:julik-frontend-races-reviewer` | Stimulus/Turbo controllers, DOM events, timers, animations, or async UI flows |

**CE conditional (migration-specific):**

| Agent | Select when diff includes migration files |
|-------|------------------------------------------|
| `ail:review:schema-drift-detector` | Cross-references schema.rb against included migrations |
| `ail:review:deployment-verification-agent` | Produces deployment checklist with SQL verification queries |

## Review Scope

Every review spawns all 4 always-on personas plus the 2 AIL always-on agents, then adds whichever cross-cutting and stack-specific conditionals fit the diff. The model naturally right-sizes: a small config change triggers 0 conditionals = 6 reviewers. A Rails auth feature might trigger security + reliability + kieran-rails + dhh-rails = 10 reviewers.

## Protected Artifacts

The following paths are ail pipeline artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer:

- `requirements/*` -- requirements documents created by ail-brainstorm
- `plans/*.md` -- plan files created by ail-plan (living documents with progress checkboxes)
...
Use the repository portion of the returned PR URL as `<base-repo>` (for example, `davidgolding/ail` from `https://github.com/davidgolding/ail/pull/348`).
...
2. **PR body.** If PR metadata was fetched in Stage 1, scan the body for paths matching `plans/*.md`. If exactly one match is found and the file exists, use it as `plan_source: explicit`. If multiple plan paths appear, treat as ambiguous — demote to `plan_source: inferred` for the most recent match that exists on disk, or skip if none exist or none clearly relate to the PR title/intent. Always verify the selected file exists before using it — stale or copied plan links in PR descriptions are common.
3. **Auto-discover.** Extract 2-3 keywords from the branch name (e.g., `feat/onboarding-skill` -> `onboarding`, `skill`). Glob `plans/*` and filter filenames containing those keywords. If exactly one match, use it. If multiple matches or the match looks ambiguous (e.g., generic keywords like `review`, `fix`, `update` that could hit many plans), **skip auto-discovery** — a wrong plan is worse than no plan. If zero matches, skip.
...
Read the diff and file list from Stage 1. The 4 always-on personas and 2 AIL always-on agents are automatic. For each cross-cutting and stack-specific conditional persona in the persona catalog included below, decide whether the diff warrants it. This is agent judgment, not keyword matching.
...
For AIL conditional agents, check if the diff includes files matching `db/migrate/*.rb`, `db/schema.rb`, or data backfill scripts.
...
1. Use the native file-search tool (e.g., Glob in Claude Code) to find `**/.ai/system.md` in the repo.
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `packages/core/.ai/system.md` applies to everything under `packages/core/`).
...
**CE conditional agents** (schema-drift-detector, deployment-verification-agent) are also dispatched as standard Agent calls when applicable. Pass the same review context bundle plus the applicability reason (for example, which migration files triggered the agent). For schema-drift-detector specifically, pass the resolved review base branch explicitly so it never assumes `main`. Their output is unstructured and must be preserved for Stage 6 synthesis just like the AIL always-on agents.
...
10. **Preserve AIL agent artifacts.** Keep the learnings, agent-native, schema-drift, and deployment-verification outputs alongside the merged finding set. Do not drop unstructured agent output just because it does not match the persona JSON schema.
...
Artifact: .context/ail/ail-review/<run-id>/
...
5. **Protected artifacts are respected.** Discard any findings that recommend deleting or gitignoring files in `requirements/`, `plans/`, or `solutions/`.
...
- In interactive, autofix, and headless modes, write a per-run artifact under `.context/ail/ail-review/<run-id>/` containing:
