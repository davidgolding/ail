...
1. **Code Review (`/ail-review`)**: If NOT already on the target branch (PR branch or requested branch), offer worktree for isolated review
2. **Feature Work (`/ail-work`)**: Always ask if user wants parallel worktree or live branch work
...
The skill is automatically called from `/ail-review` and `/ail-work` commands:
