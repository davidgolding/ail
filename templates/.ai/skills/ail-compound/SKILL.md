...
# Learnings Worth Perpetuating

Coordinate multiple subagents working in parallel to document a recently solved problem.

## Purpose

Captures problem solutions while context is fresh, creating structured documentation in `solutions/` with YAML frontmatter for searchability and future reference. Uses parallel subagents for maximum efficiency.
...
```bash
/ail-compound                    # Document the most recent fix
/ail-compound [brief context]    # Provide additional context hint
```
...
**Always run full mode by default.** Proceed directly to Phase 1 unless the user explicitly requests compact-safe mode (e.g., `/ail-compound --compact` or "use compact mode").
...
#### 1. **Context Analyzer**
   ...
   - Reads `references/yaml-schema.md` for category mapping into `solutions/`
...
#### 3. **Related Docs Finder**
   ...
   **Search strategy (grep-first filtering for efficiency):**
   ...
   2. If the problem category is clear, narrow search to the matching `solutions/<category>/` directory
...
5. Create directory if needed: `mkdir -p solutions/[category]/`
6. Write the file: either the updated existing doc or the new `solutions/[category]/[filename].md`
...
`ail-compound-refresh` is **not** a default follow-up. Use it selectively when the new learning suggests an older learning or pattern doc may now be inaccurate.

It makes sense to invoke `ail-compound-refresh` when one or more of these are true:
...
It does **not** make sense to invoke `ail-compound-refresh` when:
...
- If there is **one obvious stale candidate**, invoke `ail-compound-refresh` with a narrow scope hint after the new learning is written
- If there are **multiple candidates in the same area**, ask the user whether to run a targeted refresh for that module, category, or pattern set
- If context is already tight or you are in compact-safe mode, do not expand into a broad refresh automatically; instead recommend `ail-compound-refresh` as the next step with a scope hint

When invoking or recommending `ail-compound-refresh`, be explicit about the argument to pass. Prefer the narrowest useful scope:
...
- **Pattern filename or pattern topic** when the stale guidance lives in `solutions/patterns/`

Examples:

- `/ail-compound-refresh plugin-versioning-requirements`
- `/ail-compound-refresh payments`
- `/ail-compound-refresh performance-issues`
- `/ail-compound-refresh critical-patterns`
...
Do not invoke `ail-compound-refresh` without an argument unless the user explicitly wants a broad sweep.

Always capture the new learning first. Refresh is a targeted maintenance follow-up, not a prerequisite for documentation.

### Discoverability Check

After the learning is written and the refresh decision is made, check whether the project's instruction files would lead an agent to discover and search `solutions/` before starting work in a documented area. This runs every time — the knowledge store only compounds value when agents can find it.

1. Identify if the root-level instruction file exists (`.ai/system.md`). Read the file to determine its content. The file is the assessment and edit target. If the file does not exist, skip this check entirely.
2. Assess whether an agent reading the instruction files would learn three things:
   ...
   This is a semantic assessment, not a string match. The information could be a line in an architecture section, a bullet in a gotchas section, spread across multiple places, or expressed without ever using the exact path `solutions/`. Use judgment — if an agent would reasonably discover and use the knowledge store after reading the file, the check passes.
...
- **performance_issue** → `ail:review:performance-oracle`
- **security_issue** → `ail:review:security-sentinel`
- **database_issue** → `ail:review:data-integrity-guardian`
- Any code-heavy issue → always run `ail:review:code-simplicity-reviewer`, and additionally run the kieran reviewer that matches the repo's primary stack:
  - Ruby/Rails → also run `ail:review:kieran-rails-reviewer`
  - Python → also run `ail:review:kieran-python-reviewer`
  - TypeScript/JavaScript → also run `ail:review:kieran-typescript-reviewer`
...
3. **Write minimal doc**: Create `solutions/[category]/[filename].md` using the appropriate track template from `assets/resolution-template.md`, with:
...
Tip: Your .ai/system.md doesn't surface solutions/ to agents —
...
In compact-safe mode, the overlap check is skipped (no Related Docs Finder subagent). This means compact-safe mode may create a doc that overlaps with an existing one. That is acceptable — `ail-compound-refresh` will catch it later. Only suggest `ail-compound-refresh` if there is an obvious narrow refresh target. Do not broaden into a large refresh sweep from a compact-safe session.
...
| Multiple files created during workflow | One solution doc written or updated: `solutions/[category]/[filename].md` (plus an optional small edit to a project instruction file for discoverability) |
...
File created:
- solutions/performance-issues/n-plus-one-brief-generation.md
...
Overlap detected: solutions/performance-issues/n-plus-one-queries.md
...
File updated:
- solutions/performance-issues/n-plus-one-queries.md (added last_updated: 2026-03-24)
```
...
2. Document the solution → solutions/performance-issues/n-plus-one-briefs.md (5 min)
...
<manual_override> Use /ail-compound [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>

## Output

Writes the final learning directly into `solutions/`.
...
### Code Quality & Review
- **ail:review:kieran-rails-reviewer**: Reviews code examples for Rails best practices
- **ail:review:kieran-python-reviewer**: Reviews code examples for Python best practices
- **ail:review:kieran-typescript-reviewer**: Reviews code examples for TypeScript best practices
- **ail:review:code-simplicity-reviewer**: Ensures solution code is minimal and clear
- **ail:review:pattern-recognition-specialist**: Identifies anti-patterns or repeating issues

### Specific Domain Experts
- **ail:review:performance-oracle**: Analyzes performance_issue category solutions
- **ail:review:security-sentinel**: Reviews security_issue solutions for vulnerabilities
- **ail:review:data-integrity-guardian**: Reviews database_issue migrations and queries

### Enhancement & Research
- **ail:research:best-practices-researcher**: Enriches solution with industry best practices
- **ail:research:framework-docs-researcher**: Links to framework/library documentation references

### When to Invoke
- **Auto-triggered** (optional): Agents can run post-documentation for enhancement
- **Manual trigger**: User can invoke agents after /ail-compound completes for deeper review

## Related Commands

- `/research [topic]` - Deep investigation (searches solutions/ for patterns)
- `/ail-plan` - Planning workflow (references documented solutions)