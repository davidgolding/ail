...
`ail-ideate` precedes `ail-brainstorm`.

- `ail-ideate` answers: "What are the strongest ideas worth exploring?"
- `ail-brainstorm` answers: "What exactly should one chosen idea mean?"
- `ail-plan` answers: "How should it be built?"

This workflow produces a ranked ideation artifact in `.ai/docs/`. It does **not** produce requirements, plans, or code.
...
- a path such as `plugins/ail/skills/`
...
7. **Route action into brainstorming** - Ideation identifies promising directions; `ail-brainstorm` defines the selected one precisely enough for planning.
...
Look in `.ai/docs/` for `*-ideation.md` documents created within the last 30 days.
...
1. **Quick context scan** — dispatch a general-purpose sub-agent with this prompt:
   ...
   > Read the project's .ai/system.md (or README.md if it does not exist), then discover the top-level directory layout using the native file-search/glob tool (e.g., `Glob` with pattern `*` or `*/*` in Claude Code). Return a concise summary (under 30 lines) covering:
...
2. **Learnings search** — dispatch `ail:research:learnings-researcher` with a brief summary of the ideation focus.

3. **Issue intelligence** (conditional) — if issue-tracker intent was detected in Phase 0.2, dispatch `ail:research:issue-intelligence-analyst` with the focus hint. If a focus hint is present, pass it so the agent can weight its clustering toward that area. Run this in parallel with agents 1 and 2.
...
- **Past learnings** — relevant institutional knowledge from .ai/solutions/
...
8. When using sub-agents, assign each one a different ideation frame as a **starting bias, not a constraint**. Prompt each agent to begin from its assigned perspective but follow any promising thread wherever it leads — cross-cutting ideas that span multiple frames are valuable, not out of scope.
   ...
   **When issue-tracker intent is active and themes were returned:**
   - Each theme with `confidenail- high` or `confidenail- medium` becomes an ideation frame. The frame prompt uses the theme title and description as the starting bias.
...
- the workflow is about to hand off to `ail-brainstorm`, Proof sharing, or session end
...
- handing off to `ail-brainstorm`
- sharing to Proof
- ending the session

To write the artifact:

1. Ensure `.ai/docs/` exists
2. Choose the file path:
   - `.ai/docs/YYYY-MM-DD-<topic>-ideation.md`
   - `.ai/docs/YYYY-MM-DD-open-ideation.md` when no focus exists
...
- invoke `ail-brainstorm` with the selected idea as the seed
...
- acting on an idea routes to `ail-brainstorm`, not directly to implementation