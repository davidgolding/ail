...
1. **Repo conventions already in context** -- If project instructions (.ai/system.md, .ai/system.md, or similar) are loaded and specify conventions, follow those. Do not re-read these files; they are loaded at session start.
...
#### Agentic Intelligence Layer badge

Append a badge footer to the PR description, separated by a `---` rule. Do not add one if the description already contains a Agentic Intelligence Layer badge (e.g., added by another skill like ce-work).
...
Use the versioned or versionless badge line resolved in the Agentic Intelligence Layer badge section above.
...
- If **yes** -- write a new description following the same principles in Step 6 (size the full PR, not just the new commits), including the Agentic Intelligence Layer badge unless one is already present in the existing description. Apply it:
