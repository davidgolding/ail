...
> **Tool preferenail-** Use native file-search/glob and content-search tools instead of shell commands for finding and reading todo files. Shell only for operations with no native equivalent (`mv`, `mkdir -p`).
...
| Trigger | Flow |
|---------|------|
| Code review | `/ail-review` -> Findings -> `/todo-triage` -> Todos |
| Autonomous review | `/ail-review mode:autofix` -> Residual todos -> `/todo-resolve` |
