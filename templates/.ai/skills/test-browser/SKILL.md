...
2. **Project instructions** — check `.ai/system.md`, `.ai/system.md`, or other instruction files for port references
...
PORT="${EXPLICIT_PORT:-}"
if [ -z "$PORT" ]; then
  PORT=$(grep -Eio '(port\s*[:=]\s*|localhost:)([0-9]{4,5})' .ai/system.md 2>/dev/null | grep -Eo '[0-9]{4,5}' | head -1)
  if [ -z "$PORT" ]; then
    PORT=$(grep -Eio '(port\s*[:=]\s*|localhost:)([0-9]{4,5})' .ai/system.md 2>/dev/null | grep -Eo '[0-9]{4,5}' | head -1)
  fi
fi
