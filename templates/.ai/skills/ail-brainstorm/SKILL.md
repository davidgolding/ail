...
Brainstorming helps answer **WHAT** to build through collaborative dialogue. It precedes `/ail-plan`, which answers **HOW** to build it.

The durable output of this workflow is a **requirements document**. In other workflows this might be called a lightweight PRD or feature brief. In the Agentic Intelligence Layer, keep the workflow name `requirement`, but make the written artifact strong enough that planning does not need to invent product behavior, scope boundaries, or success criteria.
...
If the user references an existing brainstorm topic or document, or there is an obvious recent matching `*-requirements.md` file in `.ai/requirements/`:
- Read the document
...
*Constraint Check* — Check project instruction files (`.ai/system.md` and `README.md`) for workflow, product, or scope constraints that affect the brainstorm. If these add nothing, move on.

*Topic Scan* — Search for relevant terms. Read the most relevant existing artifact if one exists (requirement, plan, spec, skill, feature doc). Skim adjacent examples covering similar behavior.
...
## Next Steps
[If `Resolve Before Planning` is empty: `→ /ail-plan` for structured implementation planning]
[If `Resolve Before Planning` is not empty: `→ Resume /ail-brainstorm` to resolve blocking questions before planning]
```
...
- The visual describes implementation architecture, data schemas, state machines, or code structure (that belongs in `ail-plan`)
...
- What would `ail-plan` still have to invent if this brainstorm ended now?
...
Ensure `.ai/requirements` directory exists before writing.
...
- **Proceed to planning (Recommended)** - Run `/ail-plan` for structured implementation planning
...
Immediately run `/ail-plan` in the current session. Pass the requirements document path when one exists; otherwise pass a concise summary of the finalized brainstorm decisions. Do not print the closing summary first.

**If user selects "Proceed directly to work":**

Immediately run `/ail-work` in the current session using the finalized brainstorm output as context. If a compact requirements document exists, pass its path. Do not print the closing summary first.

**If user selects "Share to Proof":**

```bash
CONTENT=$(cat .ai/requirements/YYYY-MM-DD-<topic>-requirements.md)
...
Requirements doc: .ai/requirements/YYYY-MM-DD-<topic>-requirements.md  # if one was created

Key decisions:
- [Decision 1]
- [Decision 2]

Recommended next step: `/ail-plan`
```
...
Resume with `/ail-brainstorm` when ready to resolve these before planning.
```