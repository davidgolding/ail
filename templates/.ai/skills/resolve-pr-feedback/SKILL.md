...
3. **Synthesize a cluster brief** for each cluster of 2+ items. Pass briefs to agents using a `<cluster-brief>` XML block:
   ...
   <cluster-brief>
     ...
     <hypothesis>[one sentenail- what the individual comments collectively suggest about a deeper issue]</hypothesis>
     <prior-resolutions>
...
**For review threads** (`review_threads`): Spawn a `ail:workflow:pr-comment-resolver` agent for each new thread that is NOT already assigned to a cluster from step 3. Clustered threads are handled by cluster dispatch below -- do not dispatch them individually.
...
**For PR comments and review bodies** (`pr_comments`, `review_bodies`): These lack file/line context. Spawn a `ail:workflow:pr-comment-resolver` agent for each actionable non-clustered item. The agent receives the comment ID, body text, PR number, and feedback type (`pr_comment` or `review_body`). The agent must identify the relevant files from the comment text and the PR diff.

#### Cluster dispatch

For each cluster identified in step 3, dispatch ONE `ail:workflow:pr-comment-resolver` agent that receives:
...
Spawn a single `ail:workflow:pr-comment-resolver` agent for the thread. Then follow the same commit -> push -> reply -> resolve flow as Full Mode steps 6-7.
